...
# 1. Introduction 
In this report the election is discussed.

## 1.1 Summary data
In this chapter the data is summarized and explained how the data is collected. The percentage of votes per party per municipality were found on .... The demographics; *amount of non-west residents per municipality*, *the urban index of a municipality* and *the standardized income per municipality*, were found on the cbs site.  
...
In this heatmap the correlation between explanatory and respons variable are showed. The red color means a positive relation, the purple color means a negative relation. 
...
```
**Scatterplot**
```{r scatterplot, echo=FALSE, warning=FALSE,message=FALSE,error=FALSE}
# Two strong correlations are further explored
scatterplot(Dat_cda$High_edu_perc, Dat_cda$Mean_income) # linear relationship
```

**Normality plots**
```{r normality_plot, echo=FALSE, warning=FALSE,message=FALSE,error=FALSE}
#### Normality ####
