...
The raw respons variable is the absolute amount of residents per municipality that voted for CDA. For linear regresssion, we transformed this variable to a fraction. However, the total amount of votes per municipality is also available. Therefore, a binomial model would be a better fit to the data. A second model is developed that uses the logit as link function to transform the range of the respons. The choice for the logit was easily made. Because the inverse of the logit is directly interpretable as the log-odds ratio. This link displays the underlaying pattern of this data best. Below, the formula for the link function:

$\eta = log(\frac{\theta}{1 - \theta}) = \beta_0 + \beta_1 x_1 + \beta_2 x_2 + ... + \beta_n x_n$

Where $\theta$ is the probability of votes for CDA.

Also for logistics regression are diagnostic plots needed to visualise if any of the assumptions for the stochastic and systematic part of the model are violated. Below, is stated why the diagnostic plots are made:
  
\begin{itemize}
\item[] Check if deviance residuals are homogenic distributed
\item[] Check for influential observations
\item[] Check for multicolinearity
\end{itemize}

Most of the diagnostics from the linear model extend relatively straighforward to logistic regression. However, leverages are no longer just a function of the explanatory variable, but also depend on the respons due to iterated weighted least squares. Furthermore, $\theta$ can never be zero or one. Fortunately, this was not the case for any of the observations.

## First model

Again, the first is the full model. Stepwise backward elimination using the F-test is used to find the optimal model. 
...
By applying a quasi binomial, a dispersion parameter $\phi$ is included, resulting in larger standard errors and less significant p-values. $\phi$ is estimated on the data at 254.0441.  The variables `Frac_60plus`, `Mean_income` and factor `Non_west2` are no longer significant.

No goodness of fit test is possible because of the free dispersion parameter. Models are compared using the F-test.
```{r}
vif(glm_CDA_2)
drop1(glm_CDA_2, test = "F")
```
According to the F-test `Frac_60plus` should be removed. The values for the VIF are all very low, meaning there is barely collinearity between the explanatory variables.
...
```
The left plot display the half-normal quantiles against the pearson residuals. Ideally, these residuals would not be greater than 3. However, this plot shows residuals even up to 80. The middle plot displays the predicted values against the deviance resiuals. Also here is a large spread of the residuals observed. Also, some heterogeneity of the variance is observed. The right plots shows the cooks distance, which can identify influential observations. Three observations stand out in one or multiple plots. The observations are Amsterdam (obs 16), Dinkelland (obs 74) and Rotterdam (obs 265). Amsterdam is the municipality with the lowest percentage of CDA votes and Dinkelland has the highest percentage. 
...
The partial regression line is highly influenced by observation 16 again.

## Third model

For this model the variable `Frac_60plus` is removed, because it had the lowest F-value. Furthermore, the observations 16 (Amsterdam) and 265 (Dinkelland) are removed. Thes influenced the partial regression coefficients greatly and had large residual and cook's distances. These steps were originally done in two, but for this report they are combined.
...
summary(glm_CDA_4)
```
By removing observation 16 and 265, the factor `Non_west2` has become significant. Also the residual deviance has decreased, but is still far above the degrees of freedom.
...
The plot left still displays very large pearson residuals. The plot in the middle seems homogenic distributed, which slightly more variance for higher predicted values. And none of the observations stands out which it's cook's distance.
...
The final model is reached after dropping the variables `Mean_income` and `Frac_60plus`. Also, two observations are removed, Amsterdam (obs 16) and Dinkelland (obs 265), respectively. Removal of these observations results in partial regression slopes that represent the data best. 

All the described above is done step by step, but these are not all shown in this report. 
